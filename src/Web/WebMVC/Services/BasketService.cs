...
public class BasketService : IBasketService
{
    ...
    public BasketService(HttpClient httpClient, IOptions<AppSettings> settings, ILogger<BasketService> logger)
    {
        ...
        _basketByPassUrl = $"http://localhost:58017/api/v1/basket";
        _purchaseUrl = $"http://localhost:57425/api/v1";
    }
    ...
    public async Task AddItemToBasket(ApplicationUser user, int productId)
    {
        ...
        var content = JsonSerializer.Serialize(newItem);
        var basketContent = new StringContent(content, System.Text.Encoding.UTF8, "application/json");
        try
        {
            var response = await _apiClient.PostAsync(uri, basketContent);
            return;
